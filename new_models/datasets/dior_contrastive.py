...
DIOR_SPLIT = dict(
    ...
    ALL_CLASSES_SPLIT2 = ('golffield', 'Expressway-toll-station' ,'trainstation' ,'chimney',
                            'storagetank', 'ship', 'harbor' ,'airplane', 
                            'groundtrackfield' ,'tenniscourt', 'dam' ,'basketballcourt',
                            'Expressway-Service-area' ,'stadium', 'airport', 'baseballfield',
                            'bridge' ,'windmill', 'overpass','vehicle'),

    ALL_CLASSES_SPLIT3 = ('golffield', 'Expressway-toll-station' ,'trainstation' ,'chimney',
                            'storagetank', 'ship', 'harbor' ,'airplane', 
                            'groundtrackfield' ,'tenniscourt', 'dam' ,'basketballcourt',
                            'Expressway-Service-area' ,'stadium', 'airport', 'baseballfield',
                            'bridge' ,'windmill', 'overpass','vehicle'),
    ...
    BASE_CLASSES_SPLIT2 = ('golffield', 'Expressway-toll-station' ,'chimney',
                          'storagetank', 'ship', 'harbor' , 'groundtrackfield',
                          'dam' ,'basketballcourt', 'Expressway-Service-area' ,
                          'stadium', 'airport', 'bridge', 'overpass','vehicle'),

    BASE_CLASSES_SPLIT3 = ('golffield', 'Expressway-toll-station', 'chimney', 'storagetank', 
                            'harbor' ,'airplane', 'groundtrackfield' ,'tenniscourt',
                            'dam' ,'basketballcourt', 'Expressway-Service-area' ,'stadium', 
                            'airport', 'baseballfield', 'bridge', 'overpass',
                            'vehicle'),
)

@DATASETS.register_module()
class ContrastiveFewShotDiorDataset(BaseFewShotDataset):
    ...
    def prepare_train_img_g(self,
        ...
        if gt_idx is not None:
            ...
            results_base = dict(img_info=new_img_info, ann_info=selected_ann_info)
        # use all annotations
        else:
            results_base = dict(img_info=copy.deepcopy(img_info), ann_info=ann_info)
        ...
        if self.multi_pipelines is not None:
            ...
            # normal novel aug finish
            results = self.multi_pipelines['normal'](results_base)
            results_novel = copy.deepcopy(results)
            results = self.multi_pipelines['finish'](results)
            ...
            if pipeline_key == 'main':
                ...
                auxi = self.multi_pipelines['aux'](results_base)
        
            to_return = dict(base=results, novel=results_novel, augmented=results_augmented, img=results['img'], img_metas=results_augmented['img_metas'], auxiliary_data_list=auxi)
        ...
        for img_name in img_names:
            ...
            if 'dior' in ann_file:
                ...
                if 'val' in ann_file:
                    ...
                    ann_info, width, height = self._get_xml_ann_info_dior_hbb(img_id, classes)
    ...
    def _get_xml_ann_info_dior_hbb(self,
                          img_id: str,
...
@DATASETS.register_module()
class ContrastiveFewShotDiorDefaultDataset(ContrastiveFewShotDiorDataset):
    ...
    def __init__(self, dior_folder_path, ann_dif, ann_cfg: List[Dict], **kwargs) -> None:
        ...
        if ann_cfg[0]['method'] == 'TFA':
            self.DEFAULT_ANN_CONFIG = dict(TFA=self.dior_benchmark)

        # MPSR
        if ann_cfg[0]['method'] == 'MPSR':
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        return super(ContrastiveFewShotDiorDefaultDataset, self).ann_cfg_parser(new_ann_cfg)


@DATASETS.register_module()
class ContrastiveFewShotDiorDefaultDatasetNoDif(ContrastiveFewShotDiorDataset):
    """Dataset with some pre-defined VOC annotation paths.
            ...
            if ann_root is not None:
                for i in range(len(default_ann_cfg)):
                    default_ann_cfg[i]['ann_file'] = osp.join(
                        ann_root, default_ann_cfg[i]['ann_file'])
            new_ann_cfg += default_ann_cfg
        return super(ContrastiveFewShotDiorDefaultDatasetNoDif, self).ann_cfg_parser(new_ann_cfg)

@DATASETS.register_module()
class ContrastiveFewShotDiorCopyDefaultDatasetNoDif(ContrastiveFewShotDiorDataset):

...
class CRFCNNRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            #print("in CRFCNN ")
            bbox_results, novel_bbox_results = self._bbox_forward_train(img_features, sampling_results,
                                                    gt_nbboxes, gt_nlabels,
                                                    aug_img_features, aug_sampling_results,
                                                    gt_bboxes_aug, gt_labels_aug, gt_labels_aug_true,
                                                    img_metas, base_bbox_pred,gt_base_bbox)
            losses.update(bbox_results['loss_bbox_cont'])
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_nbboxes, gt_nlabels,
                            x_aug, aug_sampling_results, gt_bboxes_aug, gt_labels_aug, gt_labels_aug_true,
                            img_metas, base_bbox_pred,gt_base_bbox):
        ...
        aug_bbox_results = self._bbox_forward(x_aug, aug_rois)
        ...
        loss_bbox_cont = self.bbox_head.c_loss(
                x,
                gt_nbboxes,
                gt_nlabels,
                x_aug,
        ...
        aug_bbox_results.update(loss_bbox_cont=loss_bbox_cont)
        return aug_bbox_results, bbox_results
...
@HEADS.register_module()
class CRFCNNRoIHeadBU(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    """Simplest base roi head including one bbox head and one mask head."""
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            #print("in CRFCNN ")
            bbox_results = self._bbox_forward_train(img_features, sampling_results,
                                                    gt_nbboxes, gt_nlabels,
                                                    aug_img_features, aug_sampling_results,
                                                    gt_bboxes_aug, gt_labels_aug, gt_labels_aug_true,
                                                    img_metas, transform_applied)
            losses.update(bbox_results['loss_bbox_cont'])
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_nbboxes, gt_nlabels,
        ...
        rois = rois.contiguous()
        bbox_results = self._bbox_forward(x, rois)
        bbox_targets = self.bbox_head.c_get_targets(sampling_results, gt_nbboxes, gt_nlabels, self.train_cfg)
        ...
        aug_rois = aug_rois.contiguous()
        aug_bbox_results = self._bbox_forward(x_aug, aug_rois)
        aug_bbox_targets = self.bbox_head.c_get_targets(aug_sampling_results, gt_bboxes_aug, gt_labels_aug, self.train_cfg)

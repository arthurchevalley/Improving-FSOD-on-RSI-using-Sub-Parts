_base_ = [
    ...
    '../../../_base_/datasets/testing_norm_optimize_multi_rotate_s2_seeded.py',
...
model = dict(
    ...
    roi_head=dict(
        bbox_head=dict(
            type='Agnostic_QueueAugContrastiveBBoxHead_Branch_classqueue_replace_withbg',
            num_classes=nbr_classes,
            loss_bbox=dict(type='SmoothL1Loss_analyse', beta=1.0, loss_weight=10.0),
            loss_cls = dict(type='Object_CrossEntropyLoss', nbr_classes=nbr_classes,use_sigmoid=False, loss_weight=1.0),
            loss_cosine=dict(

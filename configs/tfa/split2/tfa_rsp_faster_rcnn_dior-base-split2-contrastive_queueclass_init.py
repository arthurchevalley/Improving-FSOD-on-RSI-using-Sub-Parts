_base_ = [
    ...
    '../../_base_/datasets/testing_norm_optimize_multi_rotate_base.py',
...
model = dict(
    ...
    rpn_head=dict(
            loss_cls=dict(type='CrossEntropyLoss', use_sigmoid=True, loss_weight=0.),
            loss_bbox=dict(type='SmoothL1Loss_analyse', beta = 1.0/9.0,loss_weight=0.)
    ),
    roi_head=dict(
        type='DualCosContRoIHead_Branch_novcls_nobbox_separate',
        bbox_head=dict(
            type='QueueAugContrastiveBBoxHead_Branch_classqueue',
            mlp_head_channels=128,
            with_weight_decay=True,
            main_training=True,
            same_class = True,
            num_classes=15,
            loss_cls=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=0.),
            loss_bbox=dict(type='SmoothL1Loss_analyse', beta=1.0, loss_weight=0.),
            loss_cosine=dict(
            ...
            loss_c_bbox=None,
            to_norm_cls = True,
            queue_path = 'per_class_features_base.p',
            use_queue = True,
            init_queue = True,
            use_base_queue = True,

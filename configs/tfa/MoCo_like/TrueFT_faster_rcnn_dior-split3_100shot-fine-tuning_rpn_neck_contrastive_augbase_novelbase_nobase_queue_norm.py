...
model = dict(
    ...
    roi_head=dict(
        bbox_head=dict(
            ...
            loss_c_cls=None,#dict(iou_threshold=0.8, loss_weight=0.5),
            loss_base_aug=dict(iou_threshold=0.8, loss_weight=0.5),
            loss_c_bbox=None,#dict(type='IoULossBBOX', loss_weight=0.5),
            to_norm_cls = True,
...
custom_hooks = [
    dict(type='ContrastiveLossDecayHook', decay_steps=([20000]), decay_rate=0.5)
]
log_config = dict(
    interval=50,
    hooks=[

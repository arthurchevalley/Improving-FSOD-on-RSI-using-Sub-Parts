...
class CosContRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward(self, x, base_rois, rois=None, x_aug=None, aug_rois=None):
        ...
        base_cls_score, base_bbox_pred, cls_score, bbox_pred, aug_cls_score, aug_bbox_pred = self.bbox_head(base_bbox_feats, bbox_feats, bbox_feats_aug)

        if aug_rois is not None and rois is not None:
            base_bbox_results = dict(
                cls_score=base_cls_score, bbox_pred=base_bbox_pred, bbox_feats=base_bbox_feats)
            aug_bbox_results = dict(
                cls_score=aug_cls_score, bbox_pred=aug_bbox_pred, bbox_feats=bbox_feats_aug)
